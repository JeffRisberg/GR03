...
<html>
	<head>
		...
		<p:css name="bundle3" />
		<script type="text/javascript" src="<g:createLinkTo dir='js' file='window.js' />"></script>
		<script type="text/javascript">
     function showSummary(id) {
      Dialog.alert({url: "summary/"+id, options: {method: 'get'}}, 
                {className: "ecoFrame", width: 575, height: 435,
                title: "Project Summary", okLabel: "Close"});
     }
    </script>
	</head>
	<body>
					...
					<tbody>
						<g:each in="${projectInstanceList}" status="i" var="projectInstance">
							<tr class="${(i % 2) == 0 ? 'odd' : 'even'}">					
								...
								<td>
								<a href="#" onClick="showSummary(${projectInstance.id})">
